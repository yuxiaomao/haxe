...
let generic_substitute_expr gctx e =
	...
	let rec build_expr e =
		...
		| _ ->
			map_expr_type build_expr (generic_substitute_type gctx) build_var e
		in
		CallUnification.maybe_reapply_overload_call e
	in
	build_expr e
...
let static_method_container gctx c cf p =
	...
	with Error { err_message = Module_not_found path } when path = (pack,name) ->
		...
		let mg = {
			...
			m_types = [];
			m_statics = None;
			m_extra = module_extra (s_type_path (pack,name)) m.m_extra.m_sign 0. MFake gctx.ctx.com.part_scope.compilation_step m.m_extra.m_check_policy;
		} in
		gctx.mg <- Some mg;
